---
title: "BDSH Pilot and Pre-funding Projects"
...
The BDSHub worked on a total of `r nrow(fyProjects)` pilot or pre-funding projects between `r length(unique(fyProjects$projectPIName))` different PIs during the `r fy`FY.
...
ggplot(fyProjects) + 
  ...
  theme(axis.text.x = element_text(angle = 60,
                                   hjust = 1,
                                   vjust = 1)) + 
  labs(title = sprintf("BDSHub FY %s Projects by Primary Department of PI", 
                       fy),
       x = "Department",
       y = "Total")
...
# Bar chart of the number of PIs from each department we have worked with
foo <- fyProjects[!duplicated(fyProjects[, c("projectPIName", "projectPIPrimaryDept")]), c("projectPIName", "projectPIPrimaryDept")]

# Match deparment order from plot above
a <- reorder(fyProjects$projectPIPrimaryDept,
        fyProjects$projectPIPrimaryDept,
        function(x) -length(x)) %>% 
  levels

foo$projectPIPrimaryDept <- factor(foo$projectPIPrimaryDept, levels = a)


ggplot(foo) + 
  geom_bar(aes(x = projectPIPrimaryDept)) + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 60,
                                   hjust = 1,
                                   vjust = 1)) + 
  labs(title = sprintf("FY %s Number of PIs Interacted With by Deparment", 
                       fy),
       x = "Department",
       y = "Total PIs")
...
# BDSH Effort on Pilot or Pre-Funding Projects
...
# Merge timeFY with project info
timeFY <- timeFY %>% 
  left_join(projects %>% select(projectID, projectPIPrimaryDept, projectPIName),
            by = c("timeProjectID" = "projectID"))


# Make some plots

# Plot of % effort by project total
timeFY %>% 
  group_by(timeProjectID) %>% 
  summarise(time = sum(timeInflation), .groups = "drop") %>% 
  mutate(timeProjectID = as.character(timeProjectID)) %>% 
  ggplot(aes(x = reorder(timeProjectID, time, function(x) x), y = time)) +
  ...
  labs(title = "BDSH Percent Effort of a Month by Project",
       y = "Percent Effort (% of Month)",
       x = "Projects")

cat("<br><br><br>")

# Plot of % effort by project degree
foo <- timeFY %>% 
  group_by(timeProjectID, degree) %>% 
  summarise(time = sum(timeInflation), .groups = "drop") %>% 
  right_join(expand.grid(timeProjectID = unique(timeFY$timeProjectID),
                         degree = unique(timeFY$degree)),
             by = c("timeProjectID", "degree")) %>% 
...
foo %>% 
  ...
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank()) + 
  scale_y_continuous(expand = c(0, 0)) + 
  labs(title = "BDSH MS Percent Effort of a Month by Project",
       y = "Percent Effort (% of Month)",
       x = "Projects")
...
foo %>% 
  ...
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank()) + 
  scale_y_continuous(expand = c(0, 0)) + 
  labs(title = "BDSH PhD Percent Effort of a Month by Project",
       y = "Percent Effort (% of Month)",
       x = "Projects")
...
# Do the same three plots as above, but by PI

# Fill in na PI names with missing
timeFY$projectPIName <- ifelse(is.na(timeFY$projectPIName), "Missing", timeFY$projectPIName)

# Plot of % effort by PI total
timeFY %>% 
  group_by(projectPIName) %>% 
  summarise(time = sum(timeInflation), .groups = "drop") %>% 
  ggplot(aes(x = reorder(projectPIName, time, function(x) x), y = time)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  theme_bw() +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank()) + 
  scale_y_continuous(expand = c(0, 0)) + 
  labs(title = "BDSH Total Percent Effort of a Month by PI",
       y = "Percent Effort (% of Month)",
       x = "PIs")
...
# Plot by degree
foo <- timeFY %>% 
  group_by(projectPIName, degree) %>% 
  summarise(time = sum(timeInflation), .groups = "drop") %>% 
  right_join(expand.grid(projectPIName = unique(timeFY$projectPIName),
                         degree = unique(timeFY$degree)),
             by = c("projectPIName", "degree")) %>% 
  mutate(time = ifelse(is.na(time), 0, time))
...
foo %>% 
  ...
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank()) + 
  scale_y_continuous(expand = c(0, 0)) + 
  labs(title = "BDSH MS Percent Effort of a Month by PI",
       y = "Percent Effort (% of Month)",
       x = "PIs")
...
foo %>% 
  ...
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank()) + 
  scale_y_continuous(expand = c(0, 0)) + 
  labs(title = "BDSH PhD Percent Effort of a Month by PI",
       y = "Percent Effort (% of Month)",
       x = "PIs")
