...
# # First get projectIDs for projects with corresponding FY id
# initiated <- grep(paste0("^", fyShort), projects$projectID, value = TRUE)
...
ggplot(fyProjects) + 
  ...
  labs(
    title = "BDSHub Unfunded Projects by Primary Department of PI",
    x = "Department",
    y = "Total"
  )
```
...
# Calculate % effort per month for each entry
foo <- timeFY %>% 
  group_by(timeProjectID, month, degree) %>% 
  summarise(monthHours = sum(timeInflation), .groups = "drop") %>% 
  mutate(monthPercentEffort = (monthHours/1500) * 1200)

ggplot(foo) + 
  geom_histogram(aes(x = monthPercentEffort, fill = degree))
